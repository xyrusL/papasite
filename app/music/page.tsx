...
export default function MusicPlayer() {
    ...
    // Audio Visualizer Component (Deterministic to fix hydration error)
        ...
        return (
            <div className="flex justify-center items-end h-10 sm:h-16 gap-1 w-full max-w-[200px] mx-auto absolute bottom-2 sm:bottom-4 left-1/2 -translate-x-1/2 opacity-60 pointer-events-none z-0">
                {[...Array(20)].map((_, i) => (
                    <div
                        key={i}
                        className={`w-1.5 rounded-t-sm transition-all duration-75 ${colors[i % colors.length]}`}
                        style={{
                            // Deterministic height and animation based on index to avoid hydration mismatch
    ...
    // Sparkles Component for "Vibes"
    const Sparkles = ({ isPlaying }: { isPlaying: boolean }) => (
        <div className="absolute inset-0 pointer-events-none overflow-hidden z-0">
            {/* Floating Notes - Reacts to music */}
            ...
            {/* Sparkle Dots - More active when playing */}
            {isPlaying && (
                <>
  ...
  `;
    ...
    return (
        <main className="bg-black min-h-screen text-white font-sans relative overflow-hidden flex flex-col lg:flex-row items-center justify-center p-4 lg:p-8 gap-8 selection:bg-yellow-500/30">
            ...
            <Link href="/" className="absolute top-4 left-4 lg:top-8 lg:left-8 text-gray-400 hover:text-white transition flex items-center gap-2 z-20 font-bold bg-black/20 backdrop-blur-sm px-3 py-1.5 sm:px-4 sm:py-2 rounded-full border border-white/5 hover:bg-white/10 cursor-pointer text-sm sm:text-base">
            ...
            >
                ...
                <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-[2rem] p-6 pb-8 sm:p-10 shadow-2xl w-full relative overflow-hidden">
                    ...
                    {/* Progress Bar */}
                    <div className="group w-full h-1.5 bg-gray-800 rounded-full mb-8 overflow-hidden cursor-pointer relative z-10" onClick={(e) => {
                    ...
                    <div className="flex items-center justify-between gap-4 z-10 relative">
                        <button
                            ...
                            className={`p-2 rounded-full transition-colors cursor-pointer ${isShuffle ? 'text-yellow-500 bg-yellow-500/10' : 'text-gray-500 hover:text-white hover:bg-white/5'}`}
                            title="Shuffle"
                        >
                        ...
                        <div className="flex items-center gap-4 sm:gap-6">
                            <button onClick={prevSong} className="p-2 text-xl sm:text-2xl text-gray-300 hover:text-white transition transform hover:-translate-x-1 cursor-pointer">
                                <FontAwesomeIcon icon={faBackward} />
                            </button>

                            <button
                                onClick={togglePlay}
                                className="w-14 h-14 sm:w-16 sm:h-16 bg-gradient-to-br from-yellow-400 to-yellow-600 rounded-full flex items-center justify-center text-black text-xl sm:text-2xl shadow-lg hover:scale-105 transition-transform cursor-pointer"
                            >
                                <FontAwesomeIcon icon={isPlaying ? faPause : faPlay} className="ml-1" />
                            </button>

                            <button onClick={nextSong} className="p-2 text-xl sm:text-2xl text-gray-300 hover:text-white transition transform hover:translate-x-1 cursor-pointer">
                        ...
                        <button
                            ...
                            className="p-2 rounded-full text-gray-500 hover:text-white hover:bg-white/5 transition-colors cursor-pointer"
                            title="Download Song"
                        >
            ...
            >
                <div className="flex items-center justify-between mb-4 px-2">
                    ...
                    <button
                        ...
                        className={`flex items-center gap-2 text-xs font-bold px-3 py-1.5 rounded-full transition-all border ${autoPlay ? 'bg-yellow-500/20 text-yellow-500 border-yellow-500/50' : 'bg-white/5 text-gray-500 border-white/10 hover:bg-white/10'}`}
                    >
                ...
                <div className="bg-white/5 backdrop-blur-md rounded-2xl border border-white/10 p-2 overflow-y-auto custom-scrollbar flex-1 max-h-[300px] lg:max-h-[500px]">
                    <div className="space-y-1">
                        {songs.map((song, index) => (
                            <div
                                key={index}
                                onClick={() => handleSongSelect(index)}
                                className={`p-3 rounded-lg flex items-center gap-3 cursor-pointer transition-all duration-300 group ${currentSongIndex === index ? 'bg-white/10 border border-yellow-500/30 shadow-md' : 'hover:bg-white/5 border border-transparent'}`}
                            >
                    ...
                    >
                        ...
                        <div className="flex gap-3 relative z-10">
                            <button
                                ...
                                className="flex-1 py-3 rounded-xl font-bold bg-white/5 hover:bg-white/10 text-gray-300 hover:text-white transition cursor-pointer"
                            >
                                Cancel
                            </button>
                            <button
                                onClick={confirmDownload}
                                className="flex-1 py-3 rounded-xl font-bold bg-gradient-to-r from-yellow-500 to-yellow-600 text-black hover:scale-105 transition-transform cursor-pointer"
                            >
                                Continue
                            </button>
                        </div>
